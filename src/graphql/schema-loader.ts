...
const regex =
  /\/\/\!code extended_graphql_schema\sstart([\s\S]*?)\/\/\!code extended_graphql_schema\send$/g;
      ...
      if (stats.isDirectory()) {
        ...
        if (matches) {
          const name = path
            ...
            .replace(/^[a-z]/, (_name) => _name.toUpperCase());
...
export async function getFullSchema(
  servicesPath = path.join(__dirname, '../../assets')
): Promise<any> {
  return await importSchemas(servicesPath).then((schemas) => {
    const querySchema: GraphQLSchema = convert({
      jsonSchema: Object.keys(schemas).map(
        (key) => schemas[key].extendedSchema
      ),
    });
    const mutationResolverGeneratorEntries = Object.keys(schemas)
      .map((key) =>
        Object.keys(schemas[key].mutationConfiguration).map(
          (method) => `${method}${pluralize.singular(key)}`
        )
      )
      .flat(2);
    const queryResolverGeneratorEntries = Object.keys(schemas)
      .map((key) => [
        `${pluralize.singular(camelcase(key))}`,
        `${camelcase(key)}Page`,
      ])
      .flat(2);

    return {
      fullSchema: printSchema(buildMutationType(schemas, querySchema)).replace(
        /`([^`]+)`/g,
        '"$1"'
      ),
